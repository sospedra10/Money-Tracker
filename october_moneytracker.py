...
class FinancialDataManager:
    ...
    def __init__(self, data_file: str = "financial_data.json"):
        ...
        self.categories = ["Bank", "Remuneration Account", "ETFs and Stocks", 
                          "Real Estate", "Crypto", "Reenlever", "Staking", "Others"]
...
class FinancialAnalyzer:
    ...
    def calculate_monthly_savings(self, chart_data: pd.DataFrame, 
                                category: str = "Total", 
                                months_to_analyze: int = SAVINGS_MONTHS_TO_ANALYZE) -> float:
    ...
    def project_future_wealth(self, current_amounts: Dict[str, float],
        ...
        investment_categories = ["Remuneration Account", "Real Estate", "ETFs and Stocks", "Bank", "Reenlever", "Staking"]
...
class FinancialVisualizer:
    ...
    def create_monthly_savings_bar_chart(self, chart_data: pd.DataFrame) -> go.Figure:
        ...
        last_month = chart_data['Total'].resample('ME').last().reset_index()
        # st.write(chart_data)
        # st.write('mine')
        # st.write(last_month)
...
class FinancialTrackerApp:
    ...
    def run(self):
        ...
        for category in self.data_manager.categories:
            monthly_savings[category] = self.analyzer.calculate_monthly_savings(chart_data, category, months_to_analyze=SAVINGS_MONTHS_TO_ANALYZE)
        ...
        col1, col_space, col2, col3 = st.columns([2, 2, 3, 3])
        input_years_projection = col1.number_input("Set Projected Years", 1, 50, YEARS_PROJECTION, 1)
        ...
        col3.metric(f"🚀 Projected in {YEARS_PROJECTION} Years ({MONTHS_PROJECTION} Months)", f"{future_projections[-1]:,.0f} €")
        ...
        # Anotaciones clave
        # Automatically choose representative points like [1, 3, 6, 12, 24, 36, 54], but adapting to MONTHS_PROJECTION
        indices = [1, 3, 6, 12]

        # Continue yearly (every 12 months) until reaching MONTHS_PROJECTION
        indices += list(range(24, MONTHS_PROJECTION + 1, 12))

        # Ensure we don’t exceed total months
        indices = [i for i in indices if i < MONTHS_PROJECTION]
